	...
	<div class="row full-height align-items-center">
		...
		<div class="col-sm-8 ml-5 results-content">
			<h3 class="text-center text-light">
				This query displays well-known entities and their market caps and compares their 
				average impressions to the average impressions of all entities.
			</h3>
			<table class="table table-striped table-hover">
				<thead class="thead-light">
					<tr>
						<th class="fixed-th">Advertiser</th>
						<th class="fixed-th">Market Cap</th>
						<th class="fixed-th">Advertiser's Average Impressions</th>
						<th class="fixed-th">Average Impressions of All Entities</th>
					</tr>
				</thead>
				<c:forEach items="${ result_tuples }" var="item">
					<tr>
						...
						<td class="text-light">
							<fmt:formatNumber value="${ item.getMarketCap() }" type="currency" currencySymbol="$"/>
						</td>
						<td class="text-light">${ item.getAvgImpressions() }</td>
						<td class="text-light">${ item.getAvgTotalImpressions() }</td>
					</tr>
				</c:forEach>
			</table>
		</div>
