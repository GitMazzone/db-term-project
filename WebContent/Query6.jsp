	...
	<div class="row full-height align-items-center">
		...
		<div class="col-sm-8 ml-5 results-content">
			<h3 class="text-center text-light">
				This query displays how political each entity was perceived to be, based on
				(political / not_political) votes from users of the extension.
			</h3>
			<table class="table table-striped table-hover">
				<thead class="thead-light">
					<tr>
						<th class="fixed-th">Advertiser</th>
						<th class="fixed-th">Politicalness</th>
					</tr>
				</thead>
				<c:forEach items="${ result_tuples }" var="item">
					<tr>
						<td class="text-light">${ item.getAdvertiserName() }</td>
						<td class="text-light">${ item.getPoliticalProbability() }</td>
					</tr>
				</c:forEach>
			</table>
		</div>
